...
const personsList = Array.from(new Array(100)).map((item, index, array) => {
	...
	return Object.assign({cityId: city.id, phone}, person);
});

const petsList = personsList.reduce((a, item) => {
	const petsList = Array.from(new Array(Math.floor(Math.random() * 3)), () => Object.assign({owner_id: item.id}, pets.create()));
...
const relationsList = personsList.reduce((a, item, index, array) => {
	let ids = Array
		.from(new Array(Math.floor(Math.random() * 3)), () => randomOne(personsList))
		.map(({id}) => id)
